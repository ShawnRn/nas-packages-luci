...
local QUICKSTART_UNIX = "/var/run/quickstart/local.sock"
...
local function cpu_usage_percent()
    local sys = require "luci.sys"
    local line = sys.exec("top -bn1 | sed -n '2p'") or ""
    local idle = tonumber(line:match("(%d+)%%%s+idle"))
    if idle == nil then
        return 0
    end
...
local function system_status_payload()
    local info = util.ubus("system", "info", {}) or {}
    local mem = info.memory or {}
    local total = tonumber(mem.total or 0) or 0
    local avail = tonumber(mem.available or mem.free or 0) or 0
    local available_pct = 0
    if total > 0 then
        available_pct = math.floor((avail * 100 / total) + 0.5)
    end
    ...
    return {
        ...
        cpuTemperature = temp,
        memAvailablePercentage = available_pct,
        memoryUsage = 100 - available_pct,
        localtime = current_time_text(),
        currentTime = current_time_text(),
        systemTime = current_time_text(),
        uptime = tonumber(info.uptime or 0) or 0,
        temperature = temp
    }
end
...
local function maybe_handle_local_api()
    ...
    if uri:match("/cgi%-bin/luci/istore/system/status/?$") then
        write_json({ code = 0, result = system_status_payload() })
        return true
    end
    if uri:match("/cgi%-bin/luci/istore/system/cpu/temperature/?$") then
        local temp = read_thermal_celsius()
    ...
    return function()
        ...
        elseif count + 2 <= #buffer - endp then
            ...
            if count - #output > 0 then
                ...
                count = sock:recvall(2)
            else
                count = sock:recvall(count + 2 - #buffer + endp)
            end
            if not count then
                return nil, code
            end
...
function istore_backend()
    if maybe_handle_local_api() then
        return
    end

    local sock = nixio.socket("unix", "stream")
    if sock and sock:connect(QUICKSTART_UNIX) ~= true then
        sock:close()
